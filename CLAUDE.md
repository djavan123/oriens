...
| **Contextos** | Contexto ativo (Trabalho/Casa/…) persiste em cookie (`oriens_context`, guarda `context_id` inteiro) e **filtra tudo**: Dashboard, Listas, Projetos. Tarefa/projeto sem contexto aparece em todos; lista **não** — toda lista pertence a um contexto e só aparece nele. Criáveis em Configurações. **Qualquer contexto pode ser excluído, inclusive os 4 padrão** (desde 08/2026) — a exclusão arquiva as listas vinculadas (tarefas delas voltam para "Tarefas avulsas") e solta tarefas/projetos que ainda apontavam para ele. A transição "Sair do trabalho" oferece capturar pendências. | `utils/context_utils.py`, `routes/api/context.py`, `repositories/context_repo.py` |
...
| **Listas** | Uma área de listas de tarefas, uma por vez: **Tarefas avulsas** (padrão) + **Notas** + **Repositório** (internas) + **personalizadas** (criar/renomear/arquivar). Tudo é `Task`; a lista é só agrupamento (`list_id`). **Toda lista é filha obrigatória de um contexto** (desde 08/2026) e só aparece nele — Notas/Repositório viraram 1 par por (usuário, contexto), não mais 1 por usuário. Sem contexto ativo, a tela convida a escolher um. Tarefa dentro de uma lista **herda o contexto dela** (travado no drawer, igual à herança de projeto). Tarefa com URL no título exibe o **título da página** em vez do link cru (buscado em background). Paginação (100). | `routes/lists.py`, `routes/api/lists.py`, `repositories/task_list_repo.py`, `utils/link_meta.py` |
...
**task_lists:** `id, user_id, context_id (nullable, FK contexts), name, order_index, archived, system_key (nullable), created_at, updated_at`
...
- `context_id` é nullable no banco (mesmo padrão de `tasks.context_id`/`projects.context_id`) mas **obrigatório na aplicação** (desde 08/2026): toda lista pertence a exatamente um contexto e só aparece nele — não é o padrão "NULL = aparece em todos" usado em tasks/projects. Notas/Repositório são 1 par por (usuário, contexto). Listas criadas antes da migração foram preenchidas por um backfill automático (contexto de menor id visível ao dono).
...
- Qualquer contexto (padrão ou do usuário) pode ser excluído em `/settings` (desde 08/2026). `ContextRepository.delete()` faz a limpeza manual de FK (SQLite não roda com `PRAGMA foreign_keys=ON` neste projeto): arquiva as listas vinculadas e zera `context_id` de tarefas/projetos que ainda apontavam para o contexto excluído. Se todos os contextos forem apagados, `_seed_contexts()` recria os 4 padrão no próximo boot (tabela vazia).
...
16. **Contexto obrigatório na tarefa avulsa de topo** (com ou sem lista). Tarefa dentro de lista **herda o `context_id` da lista** (campo travado no drawer, igual à herança de projeto); tarefa em "Tarefas avulsas" (sem lista) escolhe o próprio contexto. Sem nota (importância) segue valendo só para tarefa de projeto/subtarefa — avulsa de topo em qualquer lista recebe importância.
17. **Tarefa/projeto independente de contexto:** `context_id = NULL` aparece em qualquer contexto ativo (filtro: `context_id IS NULL OR context_id == ativo`). **Lista não segue essa regra** (regra 23): é sempre filha de um contexto específico, nunca "global".
...
23. **Contexto obrigatório na lista** (desde 08/2026): `POST /api/lists` exige `context_id` (validado contra os contextos do usuário). Lista não é renomeável/movível de contexto depois de criada — para mudar, arquiva e cria de novo. Excluir a lista (arquivar) solta `list_id` **e** `context_id` das tarefas dela, que voltam para "Tarefas avulsas" sem contexto.
