...
Ativar com `AI_ENABLED=true` e `AI_PROVIDER=Codex|openai` no `.env`.

| Provider | Modelo | Notas |
|---|---|---|
| `NullProvider` | — | Padrão quando AI_ENABLED=false |
| `ClaudeProvider` | Codex-sonnet-4-6 | Usa prompt caching efêmero |
...
### ✅ FASE 8 — IA
Providers desacoplados (Codex, OpenAI, Null), ativação por `.env`.
